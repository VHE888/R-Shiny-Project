...
# Function to filter gene counts based on variance and non-zero sample count
filter_counts <- function(counts, perc_var = 0, num_nonzero = 0){
  # Calculate variance for each gene and determine the variance threshold
  counts_var <- apply(counts, 1, var)
  var_percentile <- as.vector(counts_var) %>%
    quantile(probs = perc_var / 100)
  # Filter genes based on variance threshold and number of non-zero samples
  counts[counts_var >= var_percentile,] %>%
    filter(rowSums(. > 0) >= num_nonzero)
}

# Function to generate a summary table for counts filtering
...
# Function to create diagnostic plots based on variance or non-zero sample counts
diagnostic_plot <- function(counts, plot_type = c("variance", "non_zeros"), perc_var = 0, num_nonzero = 0, log_scale = FALSE){
  if(plot_type == "variance"){
    # Diagnostic plot for variance vs median expression
    ...
    # Diagnostic plot for number of non-zero samples vs median rank
    plot_data <- counts %>%
...
# Function to create a heatmap for filtered counts
filt_counts_heatmap <- function(filt_counts){
  # Log-transform counts and plot heatmap with custom legends
...
# Function to create a PCA biplot for filtered counts and metadata
pca_biplot <- function(filt_counts, metadata, first_PC = "PC1", second_PC = "PC2"){
  ...
  # Merge PCA results with metadata for plotting
